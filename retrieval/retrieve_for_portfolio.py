...
def _ranker_config(config: dict[str, object], method: str = "full_hybrid") -> RankerConfig:
    ...
    return RankerConfig(
        weights=RankingWeights(
            ...
            source_authority=float(ranking_weights.get("source_authority", 0.0)),
            source_timeliness=float(ranking_weights.get("source_timeliness", 0.0)),
            source_legal_liability=float(ranking_weights.get("source_legal_liability", 0.0)),
            source_numeric_density=float(ranking_weights.get("source_numeric_density", 0.0)),
            source_promotion_risk_penalty=float(ranking_weights.get("source_promotion_risk_penalty", 0.0)),
        ),
...
def retrieval_records(
    ...
    for item in ranked:
        ...
        record = {
            ...
            "evidence_unit_type": document.evidence_unit_type,
            "evidence_unit_index": document.evidence_unit_index,
            "evidence_unit_claim_type": document.evidence_unit_claim_type,
            ...
            "source_authority": round(float(document.source_authority_score), 6),
            "source_timeliness": round(float(document.source_timeliness_score), 6),
            "source_legal_liability": round(float(document.source_legal_liability_score), 6),
            "source_numeric_density": round(float(document.source_numeric_density_score), 6),
            "source_promotion_risk": round(float(document.source_promotion_risk_score), 6),
            "source_fetch_method": document.source_fetch_method,
            "source_update_frequency": document.source_update_frequency,
            "source_point_in_time_policy": document.source_point_in_time_policy,
            "source_documentation_url": document.source_documentation_url,
            "source_coverage_scope": document.source_coverage_scope,
            ...
            "source_authority_score": round(float(item["source_authority_score"]), 6),
            "source_timeliness_score": round(float(item["source_timeliness_score"]), 6),
            "source_legal_liability_score": round(float(item["source_legal_liability_score"]), 6),
            "source_numeric_density_score": round(float(item["source_numeric_density_score"]), 6),
            "source_promotion_risk_score": round(float(item["source_promotion_risk_score"]), 6),
            "source_quality_score": round(float(item["source_quality_score"]), 6),
